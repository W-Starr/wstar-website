  ...
  return (
    <aside className="w-full lg:w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 flex flex-col h-full lg:h-screen lg:sticky lg:top-0 z-20 overflow-y-auto">
      ...
      <nav className="flex-1 px-3 py-2 space-y-1">
        ...
        {/* Product Command Hubs */}
        <div className="pt-4 space-y-1">
          <div className="px-2 py-1 text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-600 flex items-center justify-between">
            <span>Product Command Hubs</span>
            <Sparkles className="w-3 h-3 text-purple-500" />
          </div>

          {productList.map((prod) => {
            const prodHref = `/os/products/${prod.id}`
            const isActive = pathname === prodHref || (prod.id === 'ace-acad' && pathname === '/os/ace-acad')
            const isAce = prod.id === 'ace-acad'
            const isPlant = prod.id === 'plantiq'
            ...
            return (
              <Link
                ...
                className={`flex items-center justify-between px-3 py-2 rounded-lg text-xs font-medium transition-colors ${
                  isActive
                    ? 'bg-purple-50 text-purple-700 dark:bg-purple-950/60 dark:text-purple-300 font-semibold shadow-xs'
                    : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-900 hover:text-slate-900 dark:hover:text-slate-200'
                }`}
              >
                ...
                <span className="text-[10px] font-mono text-slate-400 font-semibold px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800">
                  Hub
                </span>
              </Link>
            )
