    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="w-full max-w-2xl max-h-[92vh] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden text-slate-900 dark:text-white">
        ...
        <div className="p-4 sm:p-6 space-y-5 sm:space-y-6 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2 text-xs text-slate-400">
              ...
              {liveItem.projectId && (
                <>
                  <span>•</span>
                  <span>Project: <strong className="text-purple-600 dark:text-purple-400">{liveItem.projectId}</strong></span>
                </>
              )}
              <span>•</span>
          ...
          {(liveItem.startDate || liveItem.dueDate || liveItem.storyPoints || liveItem.estimatedHours) && (
            <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/40 border border-slate-200/80 dark:border-slate-800 text-xs">
              <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
                <Calendar className="w-4 h-4 text-blue-500" />
                <span>
                  Timeline: <strong>{liveItem.startDate || 'Start TBD'}</strong> → <strong className="text-blue-600 dark:text-blue-400">{liveItem.dueDate || 'Deadline TBD'}</strong>
                </span>
              </div>
              <div className="flex items-center gap-2 text-slate-500">
                {liveItem.storyPoints && <span className="bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded font-mono font-bold text-[11px]">{liveItem.storyPoints} pts</span>}
                {liveItem.estimatedHours && <span className="bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-2 py-0.5 rounded font-mono text-[11px]">{liveItem.estimatedHours}h est.</span>}
              </div>
            </div>
          )}

          {/* Task Dependencies / Blocked By Section */}
          {liveItem.dependencies && liveItem.dependencies.length > 0 && (
            <div className="p-3 rounded-lg bg-amber-50/70 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 space-y-2">
              <div className="flex items-center justify-between text-xs font-bold text-amber-800 dark:text-amber-300">
                <span className="flex items-center gap-1.5">
                  <span>⚠️ Prerequisite Dependencies (Blocked By)</span>
                </span>
                <span className="text-[11px] font-normal">
                  {liveItem.dependencies.every(depId => {
                    const found = workItems.find(w => w.id === depId || w.itemNumber === depId)
                    return found?.status === 'done'
                  }) ? (
                    <span className="text-emerald-600 dark:text-emerald-400 font-semibold">All Dependencies Resolved ✅</span>
                  ) : (
                    <span className="text-amber-700 dark:text-amber-400 font-medium">Blocking Execution</span>
                  )}
                </span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {liveItem.dependencies.map((depId) => {
                  const depItem = workItems.find((w) => w.id === depId || w.itemNumber === depId)
                  return (
                    <div
                      key={depId}
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border ${
                        depItem?.status === 'done'
                          ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 line-through'
                          : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 border-amber-300 dark:border-amber-700/80 shadow-xs'
                      }`}
                    >
                      <span className="font-mono font-bold">{depItem?.itemNumber || depId}</span>
                      {depItem && <span className="max-w-[200px] truncate text-[11px] text-slate-500 dark:text-slate-400">({depItem.title})</span>}
                      {depItem && <StatusBadge status={depItem.status} />}
                    </div>
                  )
                })}
              </div>
