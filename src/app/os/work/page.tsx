    ...
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          ...
          <h2 className="text-2xl font-bold font-heading text-slate-900 dark:text-white tracking-tight mt-1">
            Work Stream & Bug Tracker
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">
            Opinionated, low-friction tracking for tasks, bugs, technical debt, and features across all products.
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3 p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs">
        ...
        <select
          ...
          className="px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs text-slate-800 dark:text-slate-200 max-w-[160px] truncate"
        >
          ...
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map((item) => (
              <div
                ...
                className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-3 group transition-colors"
              >
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  ...
                  <div className="space-y-1 min-w-0 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[10px] font-semibold uppercase px-1.5 py-0.2 rounded bg-slate-100 dark:bg-slate-800 text-slate-500">
                        {item.productId}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-400 pt-0.5">
                      ...
                      {item.codeReference && (
                        <span className="font-mono text-slate-400 bg-slate-100 dark:bg-slate-800 px-1 rounded truncate max-w-xs">
                          {item.codeReference}
                        </span>
                      )}
                ...
                <div className="space-y-2.5 flex-1 overflow-y-auto">
                  {columnItems.map((item) => (
                    <div
                      key={item.id}
                      onClick={() => setSelectedItem(item)}
                      className="p-3.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xs hover:border-blue-500/50 cursor-pointer space-y-2 group transition-all"
                    >
